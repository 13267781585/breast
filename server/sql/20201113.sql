...
CREATE TABLE `breastfeed`.`question` (
  `q_id` INT NOT NULL,
  `t_id` INT NOT NULL COMMENT '问卷id',
  `q_type` INT NOT NULL COMMENT '题目类型 1单选题 0多选题',
  `q_content` VARCHAR(255) NOT NULL COMMENT '问卷题目内容',
  `q_opstions` VARCHAR(255) NOT NULL COMMENT '问卷选项，选项之间用 ‘|’ 隔开',
  PRIMARY KEY (`q_id`))
ENGINE = InnoDB
...
CREATE TABLE `breastfeed`.`answer` (
  `id` INT NOT NULL,
  `user_id` INT NOT NULL COMMENT '用户id',
  `t_id` INT NOT NULL COMMENT '问卷id',
  `answers` VARCHAR(255) NULL COMMENT '问卷选择',
  PRIMARY KEY (`id`))
COMMENT = '问卷选项';


INSERT INTO `test` VALUES (1, '婴儿洗澡如何操作', 'http://llllllllr.top/3165501_000738311796_2.jpg', '给婴儿洗澡主要注意的方方面面');

INSERT INTO `question` VALUES (1, 1, 1, '洗澡水水位应该多高？', 'A.30cm|B.60cm|C.70cm|D.80cm|E.都行');
INSERT INTO `question` VALUES (2, 1, 0, '下列洗澡水的温度哪些是合适的？', 'A.30|B.31|C.36|D.38|E.40');
INSERT INTO `question` VALUES (3, 1, 1, '幼儿在洗澡时不需要有人陪伴在旁边。', 'A.正确|B.错误');
